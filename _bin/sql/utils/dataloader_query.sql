...
SELECT key_id, game_platforms.id_game, array_remove(array_agg(game_platforms.id_platform),null) as ids_platforms
      FROM unnest(ARRAY[1,5]::integer[]) WITH ORDINALITY key_id
        LEFT JOIN game_platforms ON game_platforms.id_game=key_id
    GROUP BY ordinality, game_platforms.id_game, key_id ORDER BY ordinality